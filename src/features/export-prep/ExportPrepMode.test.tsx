...
describe('ExportPrepMode', () => {
  it('runs inference and creates editable regions', () => {
    ...
    expect(regions.some((region) => region.semanticId === 'search-input')).toBe(true);
    expect(screen.getByText('search-input')).toBeInTheDocument();
  });

  it('creates and deletes a region from the selected layer', () => {
    render(<ExportPrepMode visible onClose={() => undefined} />);

    fireEvent.click(screen.getByRole('button', { name: /Region/i }));
    expect(Object.keys(useDocumentStore.getState().document.pages[0]?.regions ?? {})).toHaveLength(1);

    fireEvent.click(screen.getByRole('button', { name: /Delete/i }));
    expect(Object.keys(useDocumentStore.getState().document.pages[0]?.regions ?? {})).toHaveLength(0);
  });
});
