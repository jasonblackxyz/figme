...
function buildFullBriefing(document: FIGMIIDocument) {
  ...
  return {
    ...
    runtimeSemantics: {
      purpose: 'Runtime regions label exportable grid areas for readme-app Design Lab. Regions are independent of the visual layers used to draw them.',
      workflow: [
        ...
        "FIGMII.regions.defineRegion({rect, componentKind:'text-input', semanticId:'search', role:'input', bindings:[{slot:'value', path:'search.query'}]})",
        'FIGMII.regions.validateRegions(pageId)',
  ...
  return {
    ...
    api: {
      ...
      convenience: [
        ...
        'getPage(id)',
        'setPageRuntime(pageId, metadata)',
        'regions.defineRegion({rect, componentKind, semanticId?, role?, bindings?, interactions?}) => regionId',
        'regions.updateRegion(id, updates)',
        'regions.listRegions(pageId?)',
        "inferRuntimeSemantics({pageIds?, strategy:'aggressive'})",
        'validateRuntimeSemantics()',
        'getPageCanvasSize(pageId?)',
        "setPageCanvasSize({cols, rows, pageId?, allowClip?})",
