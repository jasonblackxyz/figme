...
function buildFullBriefing(document: FigMeDocument) {
  ...
  return {
    ...
    layerKinds: {
      ...
      'canvas': {
        desc: 'Freeform character painting \u2014 place any characters with per-cell colors. The page background starts white; spaces inside canvas layers stay transparent. Use FigMe.paint() to create.',
        props: {
          content: "string \u2014 multiline ASCII art ('\\n'-separated). Spaces are transparent inside the layer, so lower layers or the white page show through.",
          cellColors: "Record<'row,col', {color?, bg?}> \u2014 per-cell hex colors for fg and bg. Auto-generated by paint().",
        },
      },
    },
    colorSystem: {
      description: 'All colours are specified as hex strings. The page background is white by default. Pass color and bg in addLayer() or use updateLayer() with customColors.',
    ...
    recipes: {
      ...
      operations: [
        ...
        },
        {
          ...
          notes: 'Spaces are transparent \u2014 lower layers or the white base page show through. ASCII export works natively.',
        },
        {
...
function buildRawBriefing(document: FigMeDocument) {
  ...
  return {
    ...
    gridSystem: {
      description:
        'The canvas is a 2D grid of monospace character cells. Every position is addressed by (col, row). There are no sub-cell positions. The page background is white by default. Spaces inside paint layers are transparent \u2014 lower layers or the base page show through. paint() never changes the page dimensions.',
      defaults: {
        fontFamily: document.gridConfig.fontFamily,
        fontSize: document.gridConfig.fontSize,
        lineHeight: document.gridConfig.lineHeight,
    ...
    colorSystem: {
      description: 'All colours are hex strings (e.g. \'#ffffff\', \'#1a1a2e\'). The page background starts white. Pass color (foreground) and bg (background) to paint(), and recolor the page surface explicitly with page cell overrides when needed.',
      perCell: "FigMe.stores.document.getState().setLayerCellOverridesBulk(layerId, [{row:0,col:0},{row:0,col:1}], '#ff6600')",
      pageBackground: "FigMe.stores.document.getState().setPageCellOverridesBulk([{row:0,col:0}], '#0d1117')",
    },
    recipes: {
      ...
      operations: [
        {
          name: 'Freeform painting (monochrome)',
          code: "FigMe.paint({col:2, row:2, content:'\u256d\u2500\u2500\u2500\u2500\u2500\u256e\\n\u2502 Hi  \u2502\\n\u2570\u2500\u2500\u2500\u2500\u2500\u256f', color:'#ffffff'})",
          notes: 'Spaces are transparent. The untouched page starts white. Use any Unicode characters \u2014 box-drawing (\u2500\u2502\u256d\u256e\u256f\u2570), blocks (\u2588\u2591\u2592\u2593), symbols, etc.',
        },
        {
          name: 'Freeform painting (per-span colors)',
          code: "FigMe.paint({col:2, row:2, lines:[[{text:'\u2591\u2591\u2591', color:'#3d3a34'}, {text:'\u2588\u2588\u2588', color:'#8b3a2a'}]]})",
